...
class FitrahController extends Controller
{
    ...
    {
        $fitrahs = Order::where('category_id' , 1)->get();

        $vWeek = verta()->subWeek();
        $lastWeek =Verta::parse($vWeek)->datetime();
        $week = Order::where('category_id' , 1)->where('created_at' , '>' , $lastWeek)->get();

        $vMonth = verta()->subMonth();
        $lastMonth =Verta::parse($vMonth)->datetime();
        $month = Order::where('category_id' , 1)->where('created_at' , '>' , $lastMonth)->get();

        $vYear = verta()->subYear();
        $lastYear =Verta::parse($vYear)->datetime();
        $year = Order::where('category_id' , 1)->where('created_at' , '>' , $lastYear)->get();
